...
2. Filter results to those whose `filename` ends in `.pdf` or `.docx` (case-insensitive) **and** whose `endTime` is within the last 45 seconds (`DOWNLOAD_FALLBACK_WINDOW_MS`) — enough for realistic navigate → switch to side panel → click timing, short enough that an unrelated HTML page visited afterwards rarely picks up a stale match. (Narrowed from 2 minutes during final review.) Candidates whose `finalUrl` this extension can't re-fetch — `blob:` URLs, plain non-localhost `http:` — are skipped here rather than matched and then failed, since the §4 "link has expired" message would be misleading advice for both.
...
6. The result is flagged `viaDownload: true` (the tab-URL and DOM-scrape paths don't set it), so the TL;DR system turn reads `📄 TL;DR — downloaded file "…"` instead of the plain form — the user always sees when the summary came from a download rather than the tab in front of them. The reported `url` has its query string and fragment stripped: enterprise download links (S3 presigned, SharePoint/OneDrive) routinely carry signing tokens there, and this value goes verbatim into the AI prompt via `pageCtx()` while nothing downstream ever re-fetches it.
