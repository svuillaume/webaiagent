...
**LQL reference doc retrieval (RAG-lite, `/lql/generate` only)** — `FortiCNAPP-LQL_Reference_Guide.txt` (committed to the repo, copied into the Docker image) is split into per-datasource chunks by `_load_lql_reference_chunks()` and keyword-matched against the objective by `_retrieve_lql_reference()`, injecting the top-scoring excerpts into the LQL generation system prompt. Pure stdlib keyword-density scoring, not embeddings — `serve.py` has no ML dependencies. Two things that will silently break this if touched carelessly:
1. **Header detection must not strip leading whitespace before matching.** The doc contains indented in-example mentions of datasource names (e.g. `LW_HA_FILE_CHANGES` inside a JOIN-syntax code sample) that look identical to real section headers once stripped — matching on the raw (unstripped) line at column 0 is what excludes them.
2. **Score must be length-normalized, not raw keyword count.** The last two chunks in the doc (`CloudTrailRawEvents`, `LW_ACT_GCP_ACTIVITY`) have no next-header boundary to cap them and are enormous — raw counts let them win nearly every query by sheer volume regardless of actual relevance.

Only covers the `LW_HE_*`/`LW_HA_*`/`LW_CE_*`/`LW_APA_*` families, which this doc catalogs per-datasource with full field tables. `LW_CFG_*` (AWS/Azure/GCP resource config) datasources aren't documented in this per-section format in this doc, so objectives about them (S3, IAM, EC2 config, etc.) score below the relevance threshold and get no injected excerpt — that's intentional, not a gap to fix, since the hand-curated hints already cover that family.
