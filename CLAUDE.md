...
**`extension/panel.js`** — all extension logic: gateway auth header construction, streaming chat, LQL tab, CVE lookup, CodeSec, and TL;DR. Gateway choice and model persist in `chrome.storage.local`; API key and gateway URL are session-RAM only (cleared on Chrome close, never written to disk). Ollama is the exception on both counts: its URL and model live in their own `chrome.storage.local` keys (`bf_ollama_url`/`bf_ollama_model`) — a localhost address is not a credential, and sharing the other four gateways' `bf_url`/`bf_model` slots would let the two gateway families overwrite each other. `applyGatewayProfile()` reloads `#url-input` from whichever slot the incoming gateway owns (reading storage directly, never trusting the field's current value, since the startup `storage.session`/`storage.local` callbacks race). When on a GitHub repo page, CodeSec/SBOM fetches real files via the GitHub API (recursive tree + raw content, up to 80 files, manifests prioritised); on other pages it scrapes `<pre>` blocks and guesses filenames heuristically so lacework SCA receives correct manifest names. TL;DR now extracts text from PDF/DOCX tabs directly via a vendored pdf.js (`extension/vendor/pdfjs/`, ES modules, loaded via dynamic `import()`) for PDF and a hand-rolled ZIP/XML reader for DOCX — this is the extension's first third-party dependency, which previously had none. TL;DR also has a `chrome.downloads`-based fallback (see "TL;DR download fallback" under Non-obvious runtime behaviour), which is why `manifest.json` declares the `"downloads"` permission.
...
**TL;DR download fallback** — when the current tab isn't itself a readable PDF/DOCX, TL;DR may summarize a *recently-downloaded* PDF/DOCX instead of the page in front of the user (`findRecentDocumentDownload()` in `panel.js`: newest completed download within `DOWNLOAD_FALLBACK_WINDOW_MS`, currently 45s). This exists because Chrome closes the tab after navigating straight to a `.docx` — there's no tab left to read. It's disclosed in the system turn (`📄 TL;DR — downloaded file "…"` vs. plain `📄 TL;DR — "…"`, driven by the `viaDownload` flag), but it's surprising behavior worth knowing about: on an ordinary HTML page, clicking TL;DR within 45s of downloading a PDF summarizes the PDF. The window is deliberately short for exactly this reason — don't lengthen it. Requires the `"downloads"` permission; if that API is unavailable (e.g. blocked by enterprise policy) the fallback returns nothing and TL;DR behaves as it did before the feature existed.

**`file://` access** — `manifest.json` declares `file:///*` in `host_permissions` and `file:` in CSP `connect-src` so TL;DR/Translate can `fetch()` local files (e.g. a PDF opened directly from disk). This alone is **not sufficient** — Chrome also requires the user to manually enable "Allow access to file URLs" for this extension in `chrome://extensions` (a per-install toggle that can't be granted from the manifest). Without it, `fetch()`/`chrome.scripting.executeScript` against a `file://` tab fails with "Cannot access contents of url ... Extension manifest must request permission to access this host" even though the manifest is correctly configured — if that error still appears after reloading the extension, check the toggle first.
